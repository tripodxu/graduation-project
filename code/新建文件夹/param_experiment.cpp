...
public:
    ...
    long long nextGrowTransientExtraBytes() const {
        ...
        if (n_[1] == 0 || n_[0] == B_) {
            extra = max(extra, 1LL * blockSize(1) * (long long)sizeof(int));
        }
...
struct InstantMetrics {
    ...
    int B = 0;
    int r = 0;
    double N_1_over_r_est = 0.0;
    double B_over_N_1_over_r = 0.0;
...
struct ProcessSummary {
    ...
    double avg_B = 0.0;
    double avg_B_over_N_1_over_r = 0.0;
...
static InstantMetrics captureInstant(const ResizableArray& arr) {
    ...
    m.N_1_over_r_est = pow((double)max(1, m.N), 1.0 / (double)m.r);
    m.B_over_N_1_over_r = (m.N_1_over_r_est > 0.0 ? m.B / m.N_1_over_r_est : 0.0);
    ...
    m.adjust_space_worst_bytes =
        max(m.next_grow_transient_extra_bytes, m.next_shrink_transient_extra_bytes);
    ...
    m.total_byte_util_worst =
        m.total_space_worst_bytes
            ? 100.0 * m.payload_bytes / (double)m.total_space_worst_bytes
            : 100.0;
...
static ProcessSummary analyzeGrowProcess(int r, int targetN) {
    ...
    if (s.sampled_states > 0) {
        ...
        s.avg_B = sumB / s.sampled_states;
        s.avg_B_over_N_1_over_r = sumBRatio / s.sampled_states;
    }
    return s;
}
...
static ProcessSummary analyzeShrinkProcess(int r, int targetN) {
    ...
    s.r = r;
    s.targetN = targetN;
    s.at_targetN = captureInstant(arr);
...
static string summaryCsvHeader() {
    return "phase,r,targetN,sampled_states,"
           "avg_capacity_util,avg_byte_util,avg_total_byte_util_worst,"
           "avg_transient_extra_bytes,max_transient_extra_bytes,"
           "avg_B,avg_B_over_N_1_over_r,"
           "max_capacity_util,min_capacity_util,max_byte_util,min_byte_util,"
           "B_at_targetN,N_1_over_r_est_at_targetN,B_over_N_1_over_r_at_targetN,"
           "payload_bytes_at_targetN,capacity_slots_at_targetN,capacity_bytes_at_targetN,"
           "pointer_bytes_at_targetN,resident_bytes_at_targetN,"
           "slack_slots_at_targetN,slack_bytes_at_targetN,"
           "next_grow_extra_bytes_at_targetN,next_shrink_extra_bytes_at_targetN,"
           "adjust_worst_bytes_at_targetN,total_worst_bytes_at_targetN,"
           "capacity_util_at_targetN,byte_util_at_targetN,total_byte_util_worst_at_targetN";
}

static string summaryToCsvRow(const ProcessSummary& s) {
    ostringstream out;
    out << fixed << setprecision(6)
        ...
        << s.avg_capacity_util << ','
        << s.avg_byte_util << ','
        << s.avg_total_byte_util_worst << ','
        << s.avg_transient_extra_bytes << ','
        << s.max_transient_extra_bytes << ','
        << s.avg_B << ','
        << s.avg_B_over_N_1_over_r << ','
        << s.max_capacity_util << ','
        << s.min_capacity_util << ','
        << s.max_byte_util << ','
        << s.min_byte_util << ','
        << s.at_targetN.B << ','
        << s.at_targetN.N_1_over_r_est << ','
        << s.at_targetN.B_over_N_1_over_r << ','
        << s.at_targetN.payload_bytes << ','
        << s.at_targetN.capacity_slots << ','
        << s.at_targetN.capacity_bytes << ','
        << s.at_targetN.pointer_bytes << ','
        << s.at_targetN.resident_bytes << ','
        << s.at_targetN.slack_slots << ','
        << s.at_targetN.slack_bytes << ','
        << s.at_targetN.next_grow_transient_extra_bytes << ','
        << s.at_targetN.next_shrink_transient_extra_bytes << ','
        << s.at_targetN.adjust_space_worst_bytes << ','
        << s.at_targetN.total_space_worst_bytes << ','
        << s.at_targetN.capacity_util << ','
        << s.at_targetN.byte_util << ','
        << s.at_targetN.total_byte_util_worst;
    return out.str();
}
...
static vector<int> parseIntList(const string& s) {
    ...
    if (!cur.empty()) vals.push_back(stoi(cur));
    return vals;
}

int main(int argc, char* argv[]) {
    vector<int> rList = {2,3,4,5,6,7,8};
    vector<int> NList = {1000,2000,5000,10000,20000,50000,100000,200000,500000,1000000,2000000,5000000,10000000};
    string outPrefix = "compact_exp";

    for (int i = 1; i < argc; ++i) {
        string arg = argv[i];
        if (arg.rfind("--r=", 0) == 0) rList = parseIntList(arg.substr(4));
        else if (arg.rfind("--N=", 0) == 0) NList = parseIntList(arg.substr(4));
        else if (arg.rfind("--out=", 0) == 0) outPrefix = arg.substr(6);
    }

    string summaryFile = outPrefix + "_summary.csv";
    ofstream summaryOut(summaryFile);
    summaryOut << summaryCsvHeader() << "\n";

    cerr << "Running compact experiments...\n";
    for (int r : rList) {
        for (int N : NList) {
            cerr << "[grow] r=" << r << ", N=" << N << "\n";
            ProcessSummary g = analyzeGrowProcess(r, N);
            summaryOut << summaryToCsvRow(g) << "\n";

            cerr << "[shrink] r=" << r << ", N=" << N << "\n";
            ProcessSummary s = analyzeShrinkProcess(r, N);
            summaryOut << summaryToCsvRow(s) << "\n";
        }
    }

    cerr << "Done.\n";
    cerr << "Generated: " << summaryFile << "\n";
    return 0;
}